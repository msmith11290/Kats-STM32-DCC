...
#ifndef INC_DCC_UNUSED_H_
#define INC_DCC_UNUSED_H_
...
//		_Bool	HW_Command;	// TRUE if command originated from hardware(HW), knobs, buttons etc. not from PC via USB
		...
		unsigned int cmd[2][MAX_FIFO_DEPTH];	// contains two commands, one currently in use and another being worked on
		int		buffInUse;	// Which of the two cmd[2][XX]'s currently in use
		};



#endif /* INC_DCC_UNUSED_H_ */
